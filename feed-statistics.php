...
Description: Compiles statistics about how many people are reading your WordPress blog via a feed reader and what posts they're reading.
Version: 5.0a5
...
define( 'FEED_STATISTICS_VERSION', '5.0' );
...
class FEED_STATS {
	...
	public static function init() {
		...
		add_action( 'admin_init', array( 'FEED_STATS', 'admin_init' ) );
		...
		if ( FEED_STATS::is_feed_url() ) {
			$fetcher_nicename = 'Browser';

			if ( empty( $_SERVER['HTTP_USER_AGENT'] ) ) {
				$identifier = md5( 'feed_statistics:' . $_SERVER['REMOTE_ADDR'] . ':blank' );
				$subscribers = 1;
			} else {
				...
				if ( preg_match( '/MauiBot/i', $user_agent ) ) {
					return;
				}

				$fetcher_nicename = FEED_STATS::nicename_from_ua( $user_agent );
				...
				if ( preg_match( '/([0-9]+) subscriber/Uis', $user_agent, $m ) ) {
					...
					$subscribers = $m[1];
				} elseif ( preg_match( '/users ([0-9]+);/Uis', $user_agent, $m ) ) {
					// For Yahoo!'s bot
					$identifier = str_replace( $m[1], '###', $user_agent );
					$subscribers = $m[1];
				} elseif ( preg_match( '/ ([0-9]+) readers/Uis', $user_agent, $m ) ) {
					// For LiveJournal's bot
					$identifier = str_replace( $m[1], '###', $user_agent );
					$subscribers = $m[1];
				} else {
					// Don't store plain-text IP.
					// Hash it with a salt to reduce vulnerability to a rainbow table.
					// Include the user agent to reduce vulnerability to a brute-force hash computation.
					$identifier = md5( 'feed_statistics:' . $_SERVER['REMOTE_ADDR'] . ':' . $user_agent );
					$subscribers = 1;
			...
			$wpdb->query( $wpdb->prepare( '
				INSERT INTO ' . $wpdb->prefix . 'feed_fetchers SET
					identifier=%s,
					subscribers=%d,
					feed=%s,
					fetcher_nicename=%s,
					fetch_date=NOW()
				ON DUPLICATE KEY UPDATE subscribers=VALUES(subscribers), fetch_date=NOW()',
				$identifier,
				$subscribers,
				$feed,
				$fetcher_nicename
	...
	public static function admin_init() {
		...
		if ( isset( $_POST['feed_statistics_nonce'] ) && wp_verify_nonce( $_POST['feed_statistics_nonce'], 'feed_statistics_update' ) ) {
	...
	public static function sql() {
		...
		switch ( $last_version ) {
			...
			case '4.0.1':
			case '4.1':
				$wpdb->query( 'CREATE TABLE IF NOT EXISTS ' . $wpdb->prefix . 'feed_fetchers
					(
						feed_fetcher_id bigint(20) unsigned NOT NULL AUTO_INCREMENT,
						identifier VARCHAR(255) NOT NULL DEFAULT "",
						feed VARCHAR(120) NOT NULL,
						fetch_date DATETIME NOT NULL DEFAULT "0000-00-00 00:00:00",
						subscribers int(11) NOT NULL DEFAULT "0",
						fetcher_nicename VARCHAR(50) NOT NULL,
						PRIMARY KEY (feed_fetcher_id),
						UNIQUE KEY feed_and_fetcher (identifier, feed),
						INDEX fetch_date (fetch_date),
						INDEX fetcher_nicename (fetcher_nicename)
					) ENGINE=InnoDB DEFAULT CHARSET=utf8'
				);

				// Instead of storing IP addresses as identifiers, store a salted and hashed combo of the IP and user agent.
				// This reduces possible exposure of personal information.
				$subscriber_records = $wpdb->get_results( $wpdb->prepare( 'SELECT * FROM ' . $wpdb->prefix . 'feed_subscribers WHERE `date` > DATE_SUB(NOW(), INTERVAL %d DAY)', get_option( 'feed_statistics_expiration_days' ) ) );

				foreach ( $subscriber_records as $subscriber_record ) {
					if ( preg_match( '/^[0-9a-f:\.]+$/', $subscriber_record->identifier ) ) {
						$subscriber_record->identifier = md5( 'feed_statistics:' . $subscriber_record->identifier . ':' . $subscriber_record->user_agent );
						$fetcher_nicename = 'Browser';
					} else {
						$fetcher_nicename = FEED_STATS::nicename_from_ua( $subscriber_record->identifier );
					}
					...
					$wpdb->query( $wpdb->prepare( '
						...
						$subscriber_record->identifier,
						$subscriber_record->feed,
						$subscriber_record->date,
						$fetcher_nicename,
						$subscriber_record->subscribers
					) );
				}
...
//				$wpdb->query( 'DROP TABLE ' . $wpdb->prefix . 'feed_subscribers' );
	...
	public static function is_feed_url() {
	...
	public static function how_many_subscribers() {
		global $wpdb;
		...
		$results = $wpdb->get_results(
			$wpdb->prepare(
				...
				FROM ' . $wpdb->prefix . 'feed_fetchers
				WHERE
					(
						(fetch_date > %s)
						OR
						(
							fetcher_nicename <> "Browser" AND
							fetch_date > %s
						)
					)
				ORDER BY `ident` ASC, fetch_date DESC',
	...
	public static function clickthroughs_page() {
		global $wpdb;
	...
	public static function topfeeds_page() {
		global $wpdb;
	...
	public static function postviews_page() {
		global $wpdb;
	...
	public static function feedreaders_page() {
		global $wpdb;
		...
		<div class="wrap">
			...
			$wpdb->query(
				$wpdb->prepare(
					'DELETE FROM ' . $wpdb->prefix . 'feed_fetchers WHERE fetch_date < %s',
					date( 'Y-m-d H:i:s', time() - ( 60 * 60 * 24 * get_option( 'feed_statistics_expiration_days' ) * 3 ) )
				)
			);
			...
			$results = $wpdb->get_results(
				$wpdb->prepare(
					'SELECT fetcher_nicename AS `reader`,
			...
			if ( ! empty( $results ) ) {
				foreach ( $results as $row ) {
					...
					} elseif ( preg_match( '/Navigator\/([0-9abpre\.]+)/is', $reader, $version ) ) {
						$reader = 'Netscape Navigator ' . $version[1];
					} elseif ( preg_match( '/Opera\/([0-9abpre\.]+)/is', $reader, $version ) ) {
						$reader = 'Opera ' . $version[1];
					} elseif ( preg_match( '/Flock\/([0-9abpre\.]+)/is', $reader, $version ) ) {
						$reader = 'Flock ' . $version[1];
					} elseif ( preg_match( '/(Firefox|BonEcho|GranParadiso|Aurora|Minefield)\/([0-9abpre\.]+)/is', $reader, $version ) ) {
						$reader = 'Mozilla ' . $version[1] . ' ' . $version[2];
					} elseif ( preg_match( '/MSIE ([0-9abpre\.]+)/is', $reader, $version ) ) {
						$reader = 'Internet Explorer ' . $version[1];
					} elseif ( preg_match( '/RockMelt\/([^\s\.]+)/is', $reader, $version ) ) {
						$reader = 'RockMelt ' . $version[1];
					} elseif ( preg_match( '/Chrome\/([^\s\.]+)/is', $reader, $version ) ) {
						$reader = 'Chrome ' . $version[1];
					} elseif ( preg_match( '/Safari/is', $reader ) ) {
						$reader = 'Safari';
					} elseif ( preg_match( '/Gecko/Uis', $reader ) ) {
						$reader = 'Other Mozilla browser';
					} elseif ( ! preg_match( '/Mozilla/Uis', $reader ) ) {
	...
	public static function feed_page() {
		...
		<div class="wrap">
			...
			<form method="post">
				<?php wp_nonce_field( 'feed_statistics_update', 'feed_statistics_nonce' ); ?>
	...
	public static function clickthrough_replace( $content ) {
		if ( is_feed() ) {
			$this_file = __FILE__;
	...
	public static function generate_clickthrough_url( $matches ) {
		$url = $matches[3];
	...
	public static function postview_tracker( $content ) {
		global $id;
	...
	public static function nicename_from_ua( $ua ) {
		$fetcher_nicename = substr( $ua, 0, strpos( $ua, ' ' ) );

		return $fetcher_nicename;
	}
}
...
function feed_statistics_action_links( $links, $file ) {
	if ( plugin_basename( dirname( __FILE__ ) . '/feed-statistics.php' == $file) ) {
